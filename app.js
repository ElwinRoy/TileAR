...
// Fine-tune height offset in meters (default 0 = exactly on the surface)
...
function applyPoseWithHeightOffset(mesh, poseMatrix, isHorizontal) {
  mesh.matrix.fromArray(poseMatrix);
  if (isHorizontal && heightOffset !== 0) mesh.matrix.elements[13] += heightOffset;
}

function smoothPoseMatrix(rawMatrix) {
  const poseMatrix = new THREE.Matrix4().fromArray(rawMatrix);
  const pos = new THREE.Vector3();
  const quat = new THREE.Quaternion();
  const scale = new THREE.Vector3();
  poseMatrix.decompose(pos, quat, scale);

  hitPoseSamples.push({ pos: pos.clone(), quat: quat.clone() });
  if (hitPoseSamples.length > HIT_POSE_SMOOTHING_FRAMES) hitPoseSamples.shift();

  const avgPos = new THREE.Vector3();
  for (const sample of hitPoseSamples) avgPos.add(sample.pos);
  avgPos.multiplyScalar(1 / hitPoseSamples.length);

  const avgQuat = hitPoseSamples[0].quat.clone();
  for (let i = 1; i < hitPoseSamples.length; i++) {
    avgQuat.slerp(hitPoseSamples[i].quat, 1 / (i + 1));
  }

  return new THREE.Matrix4().compose(avgPos, avgQuat.normalize(), new THREE.Vector3(1, 1, 1));
}

function isLikelyFloorPlane(planeY, viewerY) {
  // In local-floor space, floor should be near y=0; reject higher horizontal surfaces.
  if (planeY <= 0.35) return true;
  if (typeof viewerY === 'number') return planeY < (viewerY * 0.35);
  return false;
}
...
function createPlaneTileMesh(plane, poseMatrix) {
  ...
  applyPoseWithHeightOffset(mesh, poseMatrix, plane.orientation === 'horizontal');
  mesh.renderOrder = 1;
  mat.opacity = 0;
  return mesh;
}
...
function createAnchorMesh(poseMatrix) {
  ...
  mesh.matrixAutoUpdate = false;
  mesh.matrix.fromArray(poseMatrix);
  if (heightOffset !== 0) mesh.matrix.elements[13] += heightOffset;
  mesh.renderOrder = 1;
  return mesh;
}
  ...
  if (frame.detectedPlanes && frame.detectedPlanes.size > 0) {
    ...
    if (!isLocked) {
      ...
      for (const plane of frame.detectedPlanes) {
        ...
        if (isHoriz && surfaceMode !== 'wall') {
          const planeY = (pose.transform.position && typeof pose.transform.position.y === 'number')
            ? pose.transform.position.y
            : pose.transform.matrix[13];
          const viewerY = (viewerPose && viewerPose.transform && viewerPose.transform.position)
            ? viewerPose.transform.position.y
            : undefined;
          if (!isLikelyFloorPlane(planeY, viewerY)) continue;
        }
        ...
        if (trackedPlanes.has(plane)) {
          ...
          applyPoseWithHeightOffset(data.mesh, pose.transform.matrix, isHoriz);
...
function onTapPlace() {
  ...
  anchorMesh = createAnchorMesh(Array.from(reticle.matrix.elements));
...
function setupUI() {
  ...
  document.getElementById('height-slider').addEventListener('input', e => {
    ...
    const delta = heightOffset - previousHeightOffset;
    previousHeightOffset = heightOffset;

    if (anchorMesh) anchorMesh.matrix.elements[13] += delta;
    if (isLocked) {
      for (const [, data] of trackedPlanes) {
        if (data.type === 'floor') data.mesh.matrix.elements[13] += delta;
      }
    }
  });
}
