...
// Fine-tune height offset in meters (default 0 = exactly on surface)
...
function initDepthOcclusion(){
  ...
  const fsrc=`#version 300 es
    ...
    uniform sampler2D uDepthTex;uniform float uRawToMeters;
...
// ─── Plane-polygon geometry ─────────────────────────────────────────────────
// Polygon points are in plane-local XZ space; geometry.rotateX lays it flat.
...
// Apply the plane-detection pose directly — no Y override.
// heightOffset is applied as a pure world-space Y shift for fine-tuning.
function applyPoseWithHeightOffset(mesh, poseMatrix, isHorizontal) {
  mesh.matrix.fromArray(poseMatrix);
  if (isHorizontal && heightOffset !== 0) {
    mesh.matrix.elements[13] += heightOffset;
  }
}
...
function createPlaneTileMesh(plane, poseMatrix){
  ...
  applyPoseWithHeightOffset(mesh, poseMatrix, isHoriz);
  mesh.renderOrder=1;
  mat.opacity=0;  // fade in
  return mesh;
}
...
// ─── Anchor mesh (hit-test fallback, no plane detection) ────────────────────
// FIX: Rotate PlaneGeometry flat (-π/2 around X) so it lies on the floor.
// The WebXR hit-test pose matrix has Y pointing up (surface normal), so a
// PlaneGeometry (default normal = +Z) would be vertical without this rotation.
function createAnchorMesh(poseMatrix){
  ...
  // CRITICAL FIX: rotate geometry to lie flat in XZ plane
  const geo=new THREE.PlaneGeometry(sizeM,sizeM);
  geo.rotateX(-Math.PI/2);

  const mesh=new THREE.Mesh(geo,mat);
  mesh.matrixAutoUpdate=false;
  // Apply pose directly — hit-test result is already at floor level
  mesh.matrix.fromArray(poseMatrix);
  // Fine-tune height offset
...
function onXRFrame(time,frame){
    ...
    if(!isLocked){
      ...
      for(const plane of frame.detectedPlanes){
        ...
        if(trackedPlanes.has(plane)){
          ...
          // ── KEY FIX: use the plane's own pose directly, no Y override ──────
  ...
  // ── Hit-test reticle / fallback anchor ────────────────────────────────────
...
function setupUI(){
  ...
  // Height fine-tune: ±10 mm (−10mm to +10mm)
  const heightSlider=document.getElementById('height-slider');
  if(heightSlider){
    heightSlider.addEventListener('input',e=>{
      heightOffset=parseFloat(e.target.value)/1000; // mm → metres
      // Reapply immediately to anchor mesh
      if(anchorMesh)anchorMesh.matrix.elements[13]+=heightOffset;
    });
  }
